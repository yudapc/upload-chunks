...
const gcs = new Storage({
  keyFilename: "./gcp-key.json", // Path ke file kredensial GCP
});
const gcsBucketName = "bucket-name";
...
function finalizeUpload(res) {
  ...
  writeStream.end(() => {
    fs.rename(tempFilePath, finalFilePath, async (err) => {
      ...
      await gcs.bucket(gcsBucketName).upload(finalFilePath, {
        destination: "testing/" + finalFilePath,
      });
      console.log("Upload ke Google Cloud Storage berhasil.");
